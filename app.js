...
(function () {
  ...
  // ------------------------------------------------------------------
  // POR QUÉ ESTA FUNCIÓN EXISTE (bug que se corrigió):
  // A veces, sobre todo en celular, el mapa cargaba con Colima viéndose
  // diminuto en medio de una zona vacía, y ya no se podía mover ni
  // acercar. La causa: fitBounds()/getBoundsZoom() calculan el zoom
  // correcto según el tamaño EN PÍXELES que Leaflet cree que tiene el
  // contenedor #map en ese momento. Si esos cálculos corren una fracción
  // de segundo ANTES de que el navegador termine de acomodar el layout de
  // la página (algo que pasa más seguido en celular, o cuando los datos
  // cargan rapidísimo desde la caché del Service Worker), Leaflet cree que
  // el mapa mide casi 0 píxeles, y calcula un zoom absurdamente alejado
  // para "caber" en ese espacio. Peor: ese zoom alejado luego se usaba
  // para fijar el límite de zoom mínimo (setMinZoom) y el área de
  // arrastre (setMaxBounds), dejando el mapa atorado en esa vista
  // incorrecta — de ahí que no se pudiera mover.
  //
  // La corrección: 1) forzar a Leaflet a remedir el contenedor
  // (invalidateSize) justo antes de calcular cualquier zoom/límite, y
  // 2) esperar un frame de dibujo (requestAnimationFrame) para dar tiempo
  // a que el navegador termine de acomodar el layout antes de medir.
  // ------------------------------------------------------------------
  function applyInitialView() {
    requestAnimationFrame(() => {
      ...
      // Por si el layout todavía se seguía acomodando (fuentes web
      // terminando de cargar, la barra de direcciones del navegador en
      // celular ocultándose al hacer scroll, etc.), se vuelve a medir y
      // reencuadrar una vez más medio segundo después. Solo se hace si el
      // usuario todavía no tocó el mapa (userInteracted), para no
      // "jalarle" la vista de vuelta si ya la estaba moviendo a propósito.
      setTimeout(() => {
  ...
  // Se activa en cuanto el usuario arrastra o hace zoom manualmente, para
  // que la corrección automática de arriba deje de "pisar" sus acciones.
  let userInteracted = false;
  map.on("dragstart zoomstart", () => {
    userInteracted = true;
  });

  // Al rotar el celular (vertical <-> horizontal) cambia el tamaño real del
  // contenedor del mapa, y el límite mínimo de zoom calculado antes ya no
  // es el correcto para la nueva forma de la pantalla. Se vuelve a medir y
  // recalcular (pero sin mover la vista del usuario con fitBounds, solo se
  // ajustan los límites).
  ...
  // Vuelve a la vista inicial (todo el estado de Colima encuadrado). Se
  // usa tanto para el botón/logo "Recorrido" de la barra superior como
  // internamente al cargar la app.
  function goHome() {
    map.invalidateSize(false);
    fitToMainland();
  }
  ...
  function renderSecciones() {
    ...
    secciones.features.forEach((feature) => {
      ...
      // la versión anterior), se abre un pequeño popup con la información
      // de la sección (igual a lo que el mouse vería en el tooltip al
      // pasar por encima, algo que en celular nunca se ve porque no existe
      // el "hover" con el dedo) más 4 botones para calificarla.
      layer.on("click", (e) => openColorPicker(props, e.latlng));
  ...
  // Arma el HTML del popup: primero la MISMA información que muestra el
  // tooltip al pasar el mouse en escritorio (sección, municipio, distrito,
  // estado actual) — necesaria aquí porque en celular no existe el "hover"
  // con el dedo, así que este popup es la única forma de ver esos datos
  // antes de calificar — y abajo los 4 botones para elegir el color.
  // Cada botón lleva en data-color el valor a asignar ("verde"/"amarillo"/
  // "rojo", o "" para "quitar marca"). El contenedor guarda en
  // data-seccion el número de sección al que aplican estos botones — así
  // un solo listener "delegado" (ver más abajo, en la sección de wiring)
  // puede atender clics de CUALQUIER popup sin tener que crear un listener
  // por cada uno.
  function colorPickerHtml(props) {
    const secStr = String(props.seccion).padStart(4, "0");
    const nivel = aprobaciones.get(String(props.seccion));
    const info = NIVEL_INFO[nivel];
    const estadoActual = info ? info.label : "Sin marcar";
    return `
      <div class="color-picker" data-seccion="${props.seccion}">
        <div class="color-picker-title">
          <strong>Sección ${secStr}</strong><br>
          ${props.municipio_nombre} · Distrito ${props.distrito_l}<br>
  ...
  function openColorPicker(props, latlng) {
    L.popup({ closeButton: false, className: "cp-popup" })
      .setLatLng(latlng)
      .setContent(colorPickerHtml(props))
      .openOn(map);
  }
  ...
  // Tocar el logo/nombre "Recorrido" recentra el mapa sobre todo el estado
  // (útil como botón "inicio", y también como escape manual por si el
  // mapa alguna vez queda en una vista rara).
